...
一个栈顶指示器寄存器       T：指向数据栈STACK的栈顶
一个基地址寄存器              B：存放当前运行过程的数据区在STACK中的起始地址
一个程序地址寄存器           P：存放下一条要执行的指令地址
该假想机没有供运算用的寄存器。所有运算都要在数据栈STACK的栈顶两个单元之间进行，
并用运算结果取代原来的两个运算对象而保留在栈顶
活动记录：
 RA
 SL
 DL
...
注意：层次差为调用层次与定义层次的差值
    ...
    */
    private SPcode IP;    //指令寄存器I，存放当前要执行的代码
    ...
    private int stackSize=1000;//基本栈的空间
    private int stack_increment=100;//栈的每次可以扩充的空间
    private int max_stack_size=10000;//栈的最大空间
    private int[] dataStack=new int[stackSize];     //数据存储器STACK，初始值为1000
    private APcode code;            //存储器CODE，用来存放P的代码
    ...
    public void interpreter()
    {
        ...
        B=0;//基址寄存器
        T=0;//栈顶寄存器
        IP=code.getPcodeArray()[P];//code 就是语义分析的Pcode
        do{
            IP=code.getPcodeArray()[P];//IP表示每一行目标代码
            P++;
            switch(IP.getF())//获取伪操作码
            {
                ...
                case 3://STO L ，a 将数据栈栈顶的内容存入变量（相对地址为a，层次差为L）
                    ...
                    //T--;
                    break;
                case 4: //CAL L ，a 调用过程（转子指令）（入口地址为a，层次差为L）p258
                    dataStack[T]=B;//静态链，直接外层过程
                    dataStack[T+1]=getBase(B,IP.getL());//动态链，调用前运行过程
